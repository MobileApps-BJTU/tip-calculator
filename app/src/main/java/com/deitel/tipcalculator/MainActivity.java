...
public class MainActivity extends Activity
{
    ...
    private TextView peopleDisplayTextView; //shows the number of people
    ...
    private TextView each15TextView;    //shows the amount owed by each person with 15% tip
    ...
    private TextView eachCustomTextView; //shows the amount owed by each person with custom tip
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        each15TextView = (TextView) findViewById(R.id.each15TextView);
        tipCustomTextView = (TextView) findViewById(R.id.tipCustomTextView);
        totalCustomTextView =
                (TextView) findViewById(R.id.totalCustomTextView);
        eachCustomTextView = (TextView) findViewById(R.id.eachCustomTextView);

        // update GUI based on billAmount and customPercent
        amountDisplayTextView.setText(
                currencyFormat.format(billAmount));
        peopleDisplayTextView.setText(numberFormat.format(peopleNum));
    ...
    private void updateEach(){
        peopleNum = Integer.parseInt(peopleDisplayTextView.getText().toString());
        if(peopleNum < 1){
            peopleNum = 1;
        }
        double each15Pay = billAmount * (0.15 + 1) / peopleNum;
        double eachCustomPay = billAmount * (customPercent + 1) / peopleNum;

        each15TextView.setText(currencyFormat.format(each15Pay));
        eachCustomTextView.setText(currencyFormat.format(eachCustomPay));
    }
    ...
    private TextWatcher peopleEditTextWatcher = new TextWatcher() {
        ...
        public void onTextChanged(CharSequence charSequence, int i, int i2, int i3) {
            try{
                peopleNum = Integer.parseInt(charSequence.toString());
            }catch (Exception e){
                peopleNum = 1;
            }
            peopleDisplayTextView.setText(numberFormat.format(peopleNum));
