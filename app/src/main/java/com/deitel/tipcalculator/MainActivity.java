...
public class MainActivity extends Activity {
    ...
    private TextView total15PerPersonTextView; // shows total with 15% tip
    private TextView tipCustomTextView; // shows custom tip amount
    private TextView totalCustomPerPersonTextView; // shows total with custom tip
    private EditText numberInPartyEditText;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        total15PerPersonTextView = (TextView) findViewById(R.id.total15PerPersonTextView);
        tipCustomTextView = (TextView) findViewById(R.id.tipCustomTextView);
        totalCustomPerPersonTextView =
                (TextView) findViewById(R.id.totalCustomPerPersonTextView);
        numberInPartyEditText = (EditText) findViewById(R.id.number_in_party);
        ...
        numberInPartyEditText.addTextChangedListener(new TextWatcher() {
            @Override
            ...
            public void onTextChanged(CharSequence charSequence, int start, int before, int count) {
                if (count > 0) {
                    if (Integer.parseInt(charSequence.toString()) == 0) {
                        numberInPartyEditText.setError(Integer.parseInt(charSequence.toString()) + " is an invalid value");
                    } else {
                        numberInParty = Integer.parseInt(charSequence.toString());
                        updateStandard();
                        updateCustom();
                    }
                } else {
    ...
    private void updateStandard() {
        ...
        double fifteenPercentTotalPerPerson = (billAmount + fifteenPercentTip) / numberInParty;

        // display 15% tip and total formatted as currency
        tip15TextView.setText(currencyFormat.format(fifteenPercentTip));
        total15PerPersonTextView.setText(currencyFormat.format(fifteenPercentTotalPerPerson));
    ...
    private void updateCustom() {
        ...
        double customTotalPerPerson = (billAmount + customTip) / numberInParty;

        // display custom tip and total formatted as currency
        tipCustomTextView.setText(currencyFormat.format(customTip));
        totalCustomPerPersonTextView.setText(currencyFormat.format(customTotalPerPerson));
