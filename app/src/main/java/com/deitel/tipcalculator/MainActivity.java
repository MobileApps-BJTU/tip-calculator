...
public class MainActivity extends Activity
{
    ...
    private int number;   //initial custom number
    ...
    {
        custom_number=(EditText) findViewById(R.id.numberEditText);
        // calculate 15% tip and total
        try{
            String num= custom_number.getText().toString();
            number=Integer.parseInt(num);
        }catch(Exception e){
            Log.e(TAG, "an error occured while get the content...", e);
        }

        double fifteenPercentTip = billAmount * 0.15;
        double fifteenPercentPerson = billAmount/number + fifteenPercentTip/number;



        // display 15% tip and total costs in person
        tip15TextView.setText(currencyFormat.format(fifteenPercentTip));
        total15TextView.setText(currencyFormat.format(fifteenPercentPerson));
    ...
    private void updateCustom()
    {
        ...
        try{
            ...
            Log.e(TAG, "it is null", e);
        }

        // calculate the custom tip and total
        double customTip = billAmount * customPercent;
        double customTotal = billAmount/number + customTip/number;


        // display custom tip and total costs in person
